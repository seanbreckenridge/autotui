...
if sys.version_info[0] == 3 and sys.version_info[1] < 8:
    warnings.warn("autotui requres at least python3.8")
...
def get_validator(cls: Type, attr_name: str, type_validators: Dict[Type, AutoHandler]) -> Callable:
    """
    ...
    if cls in type_validators:
        return create_callable_prompt(attr_name, type_validators[cls])
    if is_primitive(cls):
        if cls == str:
            ...
            raise NotImplementedError("datetime handler not implemented")
    raise AutoTUIException(f"no way to handle prompting {cls}")


# ask first would be set if is_optional was true
def prompt_many(attr_name: str, promptfunc: Callable, container_type: Type, ask_first: bool) -> Callable:
    """
    ...
    def pm_lambda():
        ...
        continue_ = functools.partial(prompt_ask_another, for_attr=attr_name, dialog_title=f"Add another item to {attr_name}?")
...
def maybe_wrap_optional(attr_name: str, handler: Union[AutoHandler, Callable], is_optional: bool) -> Callable:
    """
    ...
    if isinstance(handler, AutoHandler):  # if user provided function/errors to catch for validation
        callf = create_callable_prompt(attr_name, handler)
...
def create_callable_prompt(attr_name: str, handler: AutoHandler) -> Callable:
    """
    Create a callable function with the informaton from a AutoHandler
    """
    return lambda: prompt_wrap_error(func=handler.func,
...
def namedtuple_prompt_funcs(
    ...
    for attr_name, param_type in sig.parameters.items():
        ...
        if attr_name in attr_validators.keys():
            ...
            validator_map[attr_name] = maybe_wrap_optional(attr_name, handler, is_optional)
        ...
        if is_supported_container(attr_type):
            ...
            container_type = strip_generic(attr_type)
            # optional is this context means maybe they dont want to add anything?
            # it shouldnt be allowed, according to typical typing usage, you'd just do:
            # x: List[int] = []
            # but since its possible, can do:
            # if optional present:
            ...
            internal: Sequence[Type] = typing.get_args(attr_type)
            assert len(internal) == 1, f"Expected 1 argument for {container_type}, got {len(internal)}"
            internal_type: Type = internal[0]
            # TODO: pass prompt_msg from internal type to prompt another kwarg??
            promptfunc: Callable = get_validator(internal_type, attr_name, type_validators)
            ...
            validator_map[attr_name] = prompt_many(attr_name, promptfunc, container_type, is_optional)
            ...
            # a: int
            # b: Optional[str]
            promptfunc: Callable = get_validator(attr_type, attr_name, type_validators)
            validator_map[attr_name] = maybe_wrap_optional(attr_name, promptfunc, is_optional)
